...
function DatosVehiculosDos(props) {
    ...
    useEffect(() => {
        if (vehiculoBorrar && !duplicar && !vehiculoDosEditar) {
    ...
    useEffect(() => {
        if (vehiculoDosEditar) {
            setMarcaVeh(marcaVehDos);
            setAnnoVeh(annoVehDos);
            setModeloVeh(modeloVehDos);
            setCilindrajesVeh(cilindrajeVehDos);
            setCarroceriaVeh(carroceriaVehDos);
            setTransmisionVeh(transmisionVehDos);
            setCombustibleVeh(combustibleVehDos);
            setTraccionVeh(traccionVehDos);
            ...
            listMarcas.forEach((row) => {
                if (Number.parseInt(row.id) === Number.parseInt(marcaVehDos)) {
                    setSelectMarca(row.text);
                }
            });

            annos.forEach((row) => {
                if (Number.parseInt(row.id) === Number.parseInt(annoVehDos)) {
                    setSelectAnno(row.anovehiculo);
                }
            });

            listModelos.forEach((row) => {
                if (Number.parseInt(row.id) === Number.parseInt(modeloVehDos)) {
            ...
            listCilindrajes.forEach((row) => {
                if (
                    Number.parseInt(row.id) ===
                    Number.parseInt(cilindrajeVehDos)
                ) {
                    setSelectCilindraje(row.cilindraje);
                }
            });

            if (transmisionVehDos == 1) {
                setSelectTransmision("Automática");
            } else if (transmisionVehDos == 2) {
                setSelectTransmision("Manual");
            } else {
                setSelectTransmision("");
            }

            if (combustibleVehDos == 1) {
                setSelectCombustible("Gasolina");
            } else if (combustibleVehDos == 2) {
                setSelectCombustible("Diesel");
            } else if (combustibleVehDos == 3) {
                setSelectCombustible("Gasolina - Gas");
            } else if (combustibleVehDos == 4) {
            ...
            } else if (traccionVehDos == 2) {
                setSelectTraccion("Tracción Trasera");
            } else if (traccionVehDos == 3) {
                setSelectTraccion("Tracción 4x4");
            } else if (traccionVehDos == 4) {
                setSelectTraccion("");
            }
    ...
    useEffect(() => {
        if (controlGrabar) {
            ...
            console.log("COMPARAR : ", vehiculosSeleccionados);
                ...
                vehiculosSeleccionados &&
                    vehiculosSeleccionados.map((items) => {
                        if (items.dato.valor === vehiculo) {
                            ...
                            setTextoMensajes(
                                "Heey, No puedes grabar dos vehícuos iguales 21!"
                            );
                            continuar = false;
                            return;
                            setcontrolGrabar(false);
            ...
            setTipoVehDos(tipoVehUno);
            setMarcaVehDos(marcaVeh);
            setAnnoVehDos(annoVeh);
            setModeloVehDos(modeloVeh);
    ...
    const handleChange = (selectedOptions) => {
        ...
        setCarrocerias(newDet);
        if (!vehiculoDosEditar && !vehiculoDosDuplicar) setcontrolGrabar(true);
    };
    ...
    const handleChangeCarroceria = (selectedOptions) => {
        ...
        if (vehiculoDosEditar || vehiculoDosDuplicar) {
    ...
    const handleChangeBrand = (selectedOptions) => {
        ...
        if (
            (vehiculoDosEditar || vehiculoDosDuplicar) &&
    ...
    const handleChangeModels = (selectedOptions) => {
        ...
        if (vehiculoDosEditar || vehiculoDosDuplicar) setEditarModelo(true);
    ...
    const editarDatosVehiculos = () => {
        ...
        setEditarMarcaVeh(marcaVehDos);
        setEditarAnnoVeh(annoVehDos);
        setEditarModeloVeh(modeloVehDos);
        setEditarCilindrajesVeh(cilindrajeVehDos);
        setEditarCarroceriaVeh(carroceriaVehDos);
        setEditarTransmisionVeh(transmisionVehDos);
        setEditarCombustibleVeh(combustibleVehDos);
        setEditarTraccionVeh(traccionVehDos);
        ...
        listCarrocerias.forEach((row) => {
            if (Number.parseInt(row.id) === Number.parseInt(carroceriaVehDos)) {
                setSelectCarroceria(row.carroceria);
            }
        });
        ...
        listCilindrajes.forEach((row) => {
            if (Number.parseInt(row.id) === Number.parseInt(cilindrajeVehDos)) {
                setSelectCilindraje(row.cilindraje);
            }
        });
    ...
    const duplicarDatosVehiculos = () => {
        ...
        if (!vehiculoDosCrear && !vehiculoDosDuplicar && !vehiculoDosEditar) {
            let count = controlAccion + 1;
            setControlAccion(count);
        }
        ...
        if (controlAccion > 0) {
            ...
            setTituloMensajes("Información del producto");
            setTextoMensajes(
                "Heey, solo una acción a la vez, crear, editar, duplicar o eliminar 23!"
            );
            return;
        }
        ...
        let duplicar = {
            tipoVeh: tipoVeh,
            marcaVeh: marcaVehDos,
            annoVeh: annoVehDos,
            modeloVeh: modeloVehDos,
            cilindrajesVeh: cilindrajeVehDos,
            carroceriaVeh: carroceriaVehDos,
            transmisionVeh: transmisionVehDos,
            combustibleVeh: combustibleVehDos,
            traccionVeh: traccionVehDos,
    ...
    const eliminaDatosVehiculos = () => {
        ...
        setAnnoVehDos(0);
        setModeloVehDos(0);
        setCarroceriaVehDos(0);
        setcilindrajeVehDos(0);
        settransmisionVehDos(0);
        setcombustibleVehDos(0);
        settraccionVehDos(0);
        setVehiculoBorrar(false);
    ...
    const validaGrabarDatosVehiculosEditar = () => {
        console.log("Marca XX: ", marcaVeh, editarMarcaVeh);
